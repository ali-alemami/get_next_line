...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/09/22 22:18:03 by aalemami          #+#    #+#             */
/*   Updated: 2025/09/22 22:18:03 by aalemami         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (BUFFER_SIZE < 1 || fd < 0)
		return (NULL);
	if (check_line(str[fd]) != -1)
		return (get_line_remake_str(&str[fd]));
	ptr = combine_str_ptr(&str[fd], &x, fd);
	if (!ptr)
		return (NULL);
	while (check_line(ptr) == -1)
	{
		str[fd] = ft_strcpy(&ptr);
		ptr = combine_str_ptr(&str[fd], &x, fd);
		if (!ptr)
			return (NULL);
		if (x == 0)
